...
const UserWrapper = (props) => {
    ...
    useEffect(()=>{
        ...
        Object.values(data.friends).map((elm,i)=>{
            if(elm.request===true){
                requestTrue.push(elm.uid)
            }
            if(elm.request===false){
                requestFalse.push(elm.uid)
            }
        })
        if(requestTrue.includes(userPageData.uid)){
            setfreinds(true)
        }
        else if( requestFalse.includes(userPageData.uid)){
            setRequest(true)
        }
        else {
            setRequest(false)
    ...
    return (
        <div className="userpage">
            ...
            <div className="userData">
                <div className="Card"> 
                    <img className="userAvavtar" src={userPageData.avatar?userPageData.avatar:"https://cdn3.iconfinder.com/data/icons/avatars-round-flat/33/avat-01-512.png"} />
